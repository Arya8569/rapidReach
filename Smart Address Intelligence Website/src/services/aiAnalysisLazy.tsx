...
class AIAnalysisService {
  ...
  private async initializeGroq() {
    ...
    try {
      console.log('=== DEBUG: Lazy loading Groq SDK ===')
      const { default: Groq } = await import('groq-sdk')
      
      this.groq = new Groq({
        apiKey: import.meta.env.VITE_GROQ_API_KEY || '',
        dangerouslyAllowBrowser: true // ✅ Fix for browser environment
      })
      
      console.log('=== DEBUG: Groq SDK initialized successfully ===')
      return this.groq
    } catch (error) {
  ...
  async analyzeAddress(input: string): Promise<AIAnalysisResult> {
    try {
      console.log('=== DEBUG: Starting AI analysis for:', input)
      ...
      console.log('=== DEBUG: AI analysis response received ===')
      
      // Parse JSON response
      const analysis = JSON.parse(content)
      
      console.log('=== DEBUG: AI analysis parsed successfully ===', analysis)
